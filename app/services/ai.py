...
async def generate_track_facts(title: str, artist: str, album: str, release_date: str = "") -> str:
    ...
    system = (
        ...
        "Если конкретный трек тебе неизвестен — напиши что знаешь про артиста (жанр, стиль, карьера), "
        "и в конце добавь строку: '🔍 <i>Про этот трек мало информации в открытых источниках</i>'. "
        "НЕ извиняйся, НЕ проси ссылки. Просто опиши что знаешь + пометка.\n\n"
...
async def generate_pre_recap_teaser(
    ...
    try:
        text, stop_reason = await _call_llm(
            system,
        ...
        if stop_reason != "end_turn":
            log.warning(f"Teaser cut off (stop_reason={stop_reason}), using fallback")
            return f"🎧 Сегодня {total_tracks} треков. Чем всё закончится? Узнаем прямо сейчас! 🥁"
...
async def generate_session_recap_blocks(
    ...
    rebel_info: str,
    killers_info: str,
    on_block_done: Callable[[str], None] | None = None,
) -> dict[str, str]:
    """Generate AI commentary blocks for session recap.

    Each block is generated by a separate Sonnet call for quality.
    on_block_done(block_name) is called when each block finishes (for progress tracking).
    ...
    async def _tracked_block(key: str, prompt: str) -> tuple[str, str | Exception]:
        """Generate a block and notify progress tracker when done."""
        try:
            result = await _generate_recap_block(user_context, prompt)
            return key, result
        except Exception as e:
            return key, e
        finally:
    ...
    task_results = await asyncio.gather(
        *[_tracked_block(k, block_prompts[k]) for k in keys],
    ...
    for item in task_results:
        if isinstance(item, Exception):
            log.error(f"Recap block gather failed: {item}")
            continue
        key, result = item
